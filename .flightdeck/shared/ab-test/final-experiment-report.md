...
**Structure beats English for readability.** Group B's lowest-scoring artifact (findings, 0.87) exceeds Group A's mean (0.74). The typed interface contract (0.96) is the highest-readability communication artifact measured across all experiments.
...
The Designer scored Group A messages based on typical English agent communication patterns, not actual message text. These estimates should be replaced with measured scores when transcripts are available. The estimated mean (0.74) could be higher or lower.
